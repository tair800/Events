...
namespace HospitalAPI.Services
{
    public static class ImagePathService
    {
        ...
        public static bool IsUploadedFile(string? imagePath)
        {
            ...
            // Check for GUID patterns (uploaded files often have GUIDs)
            var guidPattern = @"^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}";
            var hasGuidPattern = System.Text.RegularExpressions.Regex.IsMatch(imagePath, guidPattern, System.Text.RegularExpressions.RegexOptions.IgnoreCase);

            return uploadPatterns.Any(pattern => imagePath.StartsWith(pattern)) || hasGuidPattern;
        }
