...
const EmployeeDetail = () => {
    ...
    // Read more state for certificate info
    const [expandedCertificates, setExpandedCertificates] = useState({});
    const [collapsingCertificates, setCollapsingCertificates] = useState({});
    ...
    const MAX_UNIVERSITY_LENGTH = 20; // Maximum characters before showing read more
    ...
    const toggleCertificateExpansion = (certificateId) => {
        const isCurrentlyExpanded = expandedCertificates[certificateId];

        if (isCurrentlyExpanded) {
            // Start collapsing animation
            setCollapsingCertificates(prev => ({
                ...prev,
                [certificateId]: true
            }));
            ...
            }, 900); // Slightly longer to allow button animation to complete
        } else {
            // Expand immediately
            setExpandedCertificates(prev => ({
                ...prev,
    ...
    const shouldShowReadMore = (text) => {
        return text && text.length > MAX_UNIVERSITY_LENGTH;
    };

    // Check if cards need to be adjusted based on expanded text
    const checkCardsAdjustment = () => {
        const hasExpandedText = Object.values(expandedCertificates).some(isExpanded => isExpanded);
        setCardsAdjusted(hasExpandedText);
    };

    // Update cards adjustment when expanded certificates change
    useEffect(() => {
        checkCardsAdjustment();
    ...
    return (
        <div className="employee-detail-page">
                    ...
                    <div className="certificate-timeline-container">
                        ...
                        <div className={`certificate-timeline-line certificates-${employee.degrees ? Math.min(employee.degrees.length, 8) : 0}`}>
                            {employee.degrees && employee.degrees.slice(0, 8).map((degree, index) => {
                                const isExpanded = expandedCertificates[degree.id];
                                const isCollapsing = collapsingCertificates[degree.id];
                                const shouldTruncate = shouldShowReadMore(degree.universityName);
                                const truncatedText = shouldTruncate ? truncateText(degree.universityName, MAX_UNIVERSITY_LENGTH) : degree.universityName;
                                const remainingText = shouldTruncate ? degree.universityName.substring(MAX_UNIVERSITY_LENGTH) : '';
                                const remainingWords = shouldTruncate && (isExpanded || isCollapsing) ? remainingText.split(' ').filter(word => word.trim() !== '') : [];
                                ...
                                return (
                                    <React.Fragment key={degree.id}>
                                        ...
                                        <div className={`certificate-info certificate-info-${index + 1} ${isExpanded ? 'expanded' : ''} ${isCollapsing ? 'collapsing' : ''}`}>
                                            <h4 className="certificate-university">
                                                {shouldTruncate && !isExpanded ? truncatedText : degree.universityName.substring(0, MAX_UNIVERSITY_LENGTH)}
                                                {shouldTruncate && (isExpanded || isCollapsing) && (
                                                    <span className="certificate-university-expanded">
                                                        {remainingWords.map((word, wordIndex) => {
                                                            const totalWords = remainingWords.length;
                                                            const reverseDelay = (totalWords - wordIndex - 1) * 0.1;
                                                            const forwardDelay = wordIndex * 0.1;
                                                            const delay = isCollapsing ? reverseDelay : forwardDelay;

                                                            return (
                                                                <span
                                                                    key={wordIndex}
                                                                    className={`certificate-word ${isCollapsing ? 'collapsing' : ''}`}
                                                                    style={{
            ...
            <div className={`cards-section ${cardsAdjusted ? 'adjusted' : ''}`}>
