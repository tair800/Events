import { useLanguage } from '../context/LanguageContext'

const translations = {
    aze: {
        // About page
        doctors: 'Həkimlər',
        events: 'Tədbirlər',
        aboutTitle: 'Azərbaycan Hepato-Pankreato-Biliar Cərrahlar İctimai Birliyi',
        aboutDescription: 'Azərbaycan Hepato-Pankreato-Biliar Cərrahlar İctimai Birliyi, qaraciyər, öd yolları və mədəaltı vəzi xəstəliklərinin diaqnostika və cərrahiyyəsi sahəsində fəaliyyət göstərən mütəxəssisləri bir araya gətirən elmi-ictimai təşkilatdır.\n\nBirliyin əsas məqsədi HPB sahəsində bilik və təcrübə mübadiləsini təşviq etmək, peşəkar inkişafı dəstəkləmək və ölkədə bu sahənin inkişafına töhfə verməkdir. Birlik həm yerli, həm də beynəlxalq əməkdaşlıqlar quraraq seminarlar, elmi konfranslar və təlimlər təşkil edir. Gənc cərrahların və mütəxəssislərin dəstəklənməsi, müasir cərrahiyyə metodlarının tətbiqi və elmi tədqiqatların təşviqi də fəaliyyətimizin əsas istiqamətlərindəndir.\n\nAzərbaycan HPB Cərrahları İctimai Birliyi olaraq, səhiyyə sisteminə dəyər qatmaq və xəstələrin həyat keyfiyyətini artırmaq üçün peşəkar bir cəmiyyət olaraq çalışırıq.',
        // Header navigation
        home: 'Ana səhifə',
        about: 'Haqqımızda',
        events: 'Tədbirlər',
        members: 'İdarə heyəti',
        membersNav: 'Üzvlər',
        membersPageTitle: 'Üzvlər',
        membersListTitle: 'Üzvlərimiz',
        membersPosition: 'Vəzifəsi',
        membersClinic: 'İşlədiyi klinika',
        membersDefaultClinic: 'Bakı, Azərbaycan',
        membersEmpty: 'Üzv tapılmadı',
        membersLoginRequired: 'Üzvləri görmək üçün daxil olun.',
        login: 'Daxil ol',
        gallery: 'Qalereya',
        blog: 'Bloq',
        contact: 'Əlaqə',
        joinMember: 'Üzv ol',
        leaveMember: 'Üzvlükdən çıx',
        // Employee detail page
        whoAmI: 'Mən Kiməm?',
        pathToMedicine: 'Tibbə Aparan Yol',
        now: 'indi',
        more: 'Daha çox',
        less: 'Daha az',
        loadingEmployeeDetails: 'İşçi məlumatları yüklənir...',
        errorLoadingEmployee: 'İşçi yüklənərkən xəta',
        employeeNotFound: 'İşçi tapılmadı',
        backToEmployees: 'İşçilərə qayıt',
        // Employee page
        loadingEmployees: 'İşçilər yüklənir...',
        errorLoadingEmployees: 'İşçilər yüklənərkən xəta',
        retry: 'Yenidən cəhd et',
        noEmployeesFound: 'Həkim tapılmadı',
        // Event detail page
        conferenceSpeakers: 'Konfrans spikerləri',
        conferencePlanning: 'Konfrans planlaması',
        gallery: 'Qalereya',
        loadingData: 'Məlumatlar yüklənir...',
        noInformationAvailable: 'Məlumat mövcud deyil',
        noTimelineData: 'Timeline məlumatları mövcud deyil',
        // Events page
        register: 'Qeydiyyatdan keç',
        viewDetails: 'Detallı bax',
        allEvents: 'Bütün',
        events: 'Tədbirlər',
        all: 'Hamısı',
        searchEvents: 'Tədbir axtar...',
        refreshCalendar: 'Kalendarı yenilə',
        loading: 'Yüklənir...',
        error: 'Xəta',
        // Home page
        buyTicket: 'Bilet al',
        viewDetailsHome: 'Detaillı bax',
        upcomingEvents: 'Gözlənilən',
        allEventsHome: 'Tədbirlər',
        viewAll: 'Hamısına bax',
        recentEvents: 'Ən son',
        pastEvents: 'Keçmiş tədbir',
        ourMembers: 'Bizim',
        members: 'İdarə heyəti',
        price: 'Qiymət',
        noEventsFound: 'Heç bir tədbir tapılmadı',
        noPastEventsFound: 'Keçmiş tədbir tapılmadı',
        basketTitle: 'Səbət',
        basketEmpty: 'Səbət boşdur',
        basketEvent: 'Tədbir',
        basketDate: 'Tarix',
        basketPrice: 'Qiymət',
        basketStatus: 'Status',
        basketBought: 'Alındı',
        viewMore: 'Ətraflı bax',
        // Footer
        home: 'Ana səhifə',
        about: 'Haqqımızda',
        events: 'Tədbirlər',
        employees: 'İdarə heyəti',
        gallery: 'Qalereya',
        blog: 'Bloq',
        contact: 'Əlaqə',
        websiteByPrefix: 'Veb-sayt',
        websiteBySuffix: 'tərəfindən hazırlanıb.',
        copyright: 'Copywrite @2025. Bütün hüquqlar qorunur. AHBPCA',
        // Months
        january: 'Yanvar',
        february: 'Fevral',
        march: 'Mart',
        april: 'Aprel',
        may: 'May',
        june: 'İyun',
        july: 'İyul',
        august: 'Avqust',
        september: 'Sentyabr',
        october: 'Oktyabr',
        november: 'Noyabr',
        december: 'Dekabr',
        // Price
        free: 'Pulsuz',
        // Request Modal
        sendRequest: 'Müraciət Göndər',
        firstName: 'Ad',
        lastName: 'Soyad',
        gender: 'Cinsiyyət',
        male: 'Kişi',
        female: 'Qadın',
        role: 'Status',
        roleDoctorSpecialist: 'Həkim-mütəxəssis',
        roleDoctor: 'Həkim',
        roleResident: 'Rezident',
        roleStudent: 'Tələbə',
        roleNurse: 'Tibb bacısı',
        specialty: 'İxtisas',
        specialtyGeneralSurgeon: 'Ümumi cərrah',
        specialtyGastroenterologist: 'Gastroenteroloq',
        specialtyRadiologist: 'Radioloq',
        specialtyOncologist: 'Onkoloq',
        specialtyPathologist: 'Patoloq',
        specialtyOther: 'Digər ixtisas',
        specialtyOtherText: 'Digər',
        sector: 'Çalışdığınız sektor',
        sectorState: 'Dövlət',
        sectorPrivate: 'Özəl',
        institution: 'Çalışdığınız qurum',
        email: 'Email',
        phone: 'Telefon',
        enterFirstName: 'Adınızı daxil edin',
        enterLastName: 'Soyadınızı daxil edin',
        enterInstitution: 'Qurumunuzu daxil edin',
        enterPhone: 'Telefon nömrənizi daxil edin',
        enterSpecialtyOther: 'Digər ixtisasınızı daxil edin',
        selectRole: 'Status seçin',
        selectSpecialty: 'İxtisas seçin',
        selectSector: 'Sektor seçin',
        firstNameRequired: 'Ad tələb olunur',
        lastNameRequired: 'Soyad tələb olunur',
        genderRequired: 'Cinsiyyət tələb olunur',
        roleRequired: 'Status tələb olunur',
        specialtyRequired: 'İxtisas tələb olunur',
        specialtyOtherRequired: 'Digər ixtisas tələb olunur',
        sectorRequired: 'Sektor tələb olunur',
        institutionRequired: 'Qurum tələb olunur',
        emailRequired: 'Email tələb olunur',
        phoneRequired: 'Telefon nömrəsi tələb olunur',
        invalidEmail: 'Zəhmət olmasa düzgün email ünvanı daxil edin',
        cancel: 'Ləğv et',
        sending: 'Göndərilir...',
        requestSentSuccess: 'Müraciət uğurla göndərildi!',
        requestRecorded: 'Müraciətiniz qeydə alındı. Tezliklə sizinlə əlaqə saxlayacağıq.',
        ok: 'Tamam',
        requestError: 'Xəta!',
        requestNotSent: 'Müraciət göndərilmədi. Zəhmət olmasa yenidən cəhd edin.',
        // Contact Page
        contactTitle: 'Nə sualın varsa,',
        contactSubtitle: 'buradayıq!',
        loadingContactInfo: 'Əlaqə məlumatları yüklənir...',
        errorLoadingContactInfo: 'Əlaqə məlumatları yüklənərkən xəta',
        checkApiServer: 'API serverinin işlədiyini yoxlayın',
        noContactInfo: 'Əlaqə məlumatları mövcud deyil',
        noSocialMedia: 'Sosial media linkləri mövcud deyil',
        loadingScene: '3D səhnə yüklənir...',
        failedToLoadScene: '3D səhnə yüklənmədi',
        retry: 'Yenidən cəhd et',
        firstName: 'Ad',
        lastName: 'Soyad',
        email: 'E-poçt',
        phone: 'Telefon',
        message: 'Mesajınız',
        enterFirstName: 'Adınızı daxil edin',
        enterLastName: 'Soyadınızı daxil edin',
        enterEmail: 'Elektron poçtunuzu daxil edin',
        enterPhone: 'Telefon nömrənizi daxil edin',
        enterMessage: 'Mesajınızı daxil edin',
        firstNameRequired: 'Ad tələb olunur',
        lastNameRequired: 'Soyad tələb olunur',
        emailRequired: 'E-poçt tələb olunur',
        phoneRequired: 'Telefon nömrəsi tələb olunur',
        messageRequired: 'Mesaj tələb olunur',
        invalidEmail: 'Düzgün e-poçt ünvanı daxil edin',
        sending: 'Göndərilir...',
        sendMessage: 'Mesaj Göndər',
        messageSentSuccess: 'Mesajınız uğurla göndərildi!',
        messageRecorded: 'Mesajınız qeydə alındı. Tezliklə sizinlə əlaqə saxlayacağıq.',
        ok: 'Tamam',
        messageError: 'Xəta!',
        messageNotSent: 'Mesaj göndərilmədi. Zəhmət olmasa yenidən cəhd edin.',
        findUsOnMap: 'Xəritədə Bizi Tapın',
        languages: 'Dillər',
        // Account Benefits
        membershipRequiredMessage: 'Elmi materiallara daxil olmaq üçün üzv olmalısınız.',
        noScientificMaterials: 'Elmi materiallar mövcud deyil.',
        loadingMaterials: 'Yüklənir...',
        // Auth Pages
        loginTitle: 'Daxil olun',
        loginFailed: 'Daxil olmaq mümkün olmadı',
        emailLabel: 'Email',
        emailPlaceholder: 'E-mailinizi daxil edin',
        passwordLabel: 'Şifrə',
        passwordPlaceholder: 'Şifrənizi daxil edin',
        forgotPasswordLink: 'Şifrənizi unutmusunuz?',
        loggingIn: 'Daxil olunur...',
        loginButton: 'Daxil ol',
        noAccount: 'Hesabınız yoxdur?',
        registerLink: 'Qeydiyyatdan keçin',
        forgotPasswordTitle: 'Şifrəni Unutmusunuz?',
        resetLinkSent: 'Şifrə sıfırlama linki e-poçtunuza göndərildi. Zəhmət olmasa poçt qutunuzu yoxlayın.',
        failedToSendLink: 'Sıfırlama linki göndərilmədi',
        sending: 'Göndərilir...',
        sendResetLink: 'Reset Link Göndər',
        backToLogin: 'Geri dön',
        registerTitle: 'Qeydiyyatdan Keçin',
        passwordsDoNotMatch: 'Şifrələr uyğun gəlmir',
        invalidEmailFormat: 'Yanlış email formatı',
        invalidPhoneFormat: 'Yanlış telefon formatı. Nümunə: +994501234567',
        finMustBe7Chars: 'FIN 7 simvol olmalıdır',
        locationRequired: 'Ünvan tələb olunur',
        accountCreated: 'Hesab yaradıldı! Daxil ol səhifəsinə yönləndirilirsiniz...',
        registrationFailed: 'Qeydiyyat uğursuz oldu',
        registering: 'Qeydiyyat...',
        haveAccount: 'Hesabınız var?',
        loginLink: 'Daxil olun',
        firstNameLabel: 'Ad',
        firstNamePlaceholder: 'Adınızı daxil edin',
        lastNameLabel: 'Soyad',
        lastNamePlaceholder: 'Soyadınızı daxil edin',
        phoneLabel: 'Telefon',
        phonePlaceholder: 'Telefon nömrənizi daxil edin',
        positionLabel: 'Vəzifəsi',
        positionPlaceholder: 'Vəzifənizi daxil edin',
        finCodeLabel: 'FIN kodu',
        finCodePlaceholder: 'Şəxsiyyət vəsiqənizin FIN kodunu daxil edin',
        locationLabel: 'Ünvan',
        locationPlaceholder: 'Ünvanınızı daxil edin',
        clinicLabel: 'Klinika',
        clinicPlaceholder: 'Klinikanızı daxil edin (istəyə bağlı)',
        passwordConfirmLabel: 'Şifrəni təsdiqləyin',
        passwordConfirmPlaceholder: 'Şifrənizi daxil edin',
        resetPasswordTitle: 'Yeni Şifrə Təyin Edin',
        invalidResetToken: 'Yanlış və ya mövcud olmayan sıfırlama tokeni',
        passwordResetSuccess: 'Şifrə uğurla sıfırlandı! Daxil ol səhifəsinə yönləndirilirsiniz...',
        resetPasswordFailed: 'Şifrə sıfırlanmadı',
        resetting: 'Yenilənir...',
        resetPasswordButton: 'Şifrəni Yenilə',
        newPasswordLabel: 'Yeni Şifrə',
        newPasswordPlaceholder: 'Yeni şifrənizi daxil edin',
        confirmPasswordLabel: 'Şifrəni Təsdiqləyin',
        confirmPasswordPlaceholder: 'Şifrənizi yenidən daxil edin',
        passwordMinLength: 'Şifrə ən azı 6 simvol olmalıdır'
    },
    en: {
        // About page
        doctors: 'Doctors',
        events: 'Events',
        aboutTitle: 'Azerbaijan Hepato-Pancreato-Biliary Surgeons Public Association',
        aboutDescription: 'The Azerbaijan Hepato-Pancreato-Biliary Surgeons Public Association is a scientific-public organization that brings together specialists working in the field of diagnosis and surgery of liver, biliary tract and pancreatic diseases.\n\nThe main goal of the Association is to promote knowledge and experience exchange in the HPB field, support professional development and contribute to the development of this field in the country. The Association organizes seminars, scientific conferences and training by establishing both local and international collaborations. Supporting young surgeons and specialists, implementing modern surgical methods and promoting scientific research are also among the main directions of our activities.\n\nAs the Azerbaijan HPB Surgeons Public Association, we work as a professional society to add value to the healthcare system and improve the quality of life of patients.',
        // Header navigation
        home: 'Home',
        about: 'About',
        events: 'Events',
        members: 'Management',
        membersNav: 'Members',
        membersPageTitle: 'Members',
        membersListTitle: 'Our Members',
        membersPosition: 'Position',
        membersClinic: 'Clinic',
        membersDefaultClinic: 'Baku, Azerbaijan',
        membersEmpty: 'No members found',
        membersLoginRequired: 'Please log in to view members.',
        login: 'Login',
        gallery: 'Gallery',
        blog: 'Blog',
        contact: 'Contact',
        joinMember: 'Join Member',
        leaveMember: 'Leave Member',
        // Employee detail page
        whoAmI: 'Who Am I?',
        pathToMedicine: 'Path to Medicine',
        now: 'now',
        more: 'More',
        less: 'Less',
        loadingEmployeeDetails: 'Loading employee details...',
        errorLoadingEmployee: 'Error loading employee',
        employeeNotFound: 'Employee not found',
        backToEmployees: 'Back to Employees',
        // Employee page
        loadingEmployees: 'Loading employees...',
        errorLoadingEmployees: 'Error loading employees',
        retry: 'Retry',
        noEmployeesFound: 'No employees found',
        // Event detail page
        conferenceSpeakers: 'Conference Speakers',
        conferencePlanning: 'Conference Planning',
        gallery: 'Gallery',
        loadingData: 'Loading data...',
        noInformationAvailable: 'No information available',
        noTimelineData: 'No timeline data available',
        // Events page
        register: 'Register',
        viewDetails: 'View Details',
        allEvents: 'All',
        events: 'Events',
        all: 'All',
        searchEvents: 'Search events...',
        refreshCalendar: 'Refresh Calendar',
        loading: 'Loading...',
        error: 'Error',
        // Home page
        buyTicket: 'Buy Ticket',
        viewDetailsHome: 'View Details',
        upcomingEvents: 'Upcoming',
        allEventsHome: 'Events',
        viewAll: 'View All',
        recentEvents: 'Recent',
        pastEvents: 'Past Events',
        ourMembers: 'Our',
        members: 'Management',
        price: 'Price',
        noEventsFound: 'No events found',
        noPastEventsFound: 'No past events found',
        basketTitle: 'Basket',
        basketEmpty: 'Basket is empty',
        basketEvent: 'Event',
        basketDate: 'Date',
        basketPrice: 'Price',
        basketStatus: 'Status',
        basketBought: 'Purchased',
        viewMore: 'View more',
        // Footer
        home: 'Home',
        about: 'About Us',
        events: 'Events',
        employees: 'Management',
        gallery: 'Gallery',
        blog: 'Blog',
        contact: 'Contact',
        websiteByPrefix: 'Website developed by',
        websiteBySuffix: '.',
        copyright: 'Copyright @2025. All rights reserved. AHBPCA',
        // Months
        january: 'January',
        february: 'February',
        march: 'March',
        april: 'April',
        may: 'May',
        june: 'June',
        july: 'July',
        august: 'August',
        september: 'September',
        october: 'October',
        november: 'November',
        december: 'December',
        // Price
        free: 'Free',
        // Request Modal
        sendRequest: 'Send Request',
        firstName: 'First Name',
        lastName: 'Last Name',
        gender: 'Gender',
        male: 'Male',
        female: 'Female',
        role: 'Role',
        roleDoctorSpecialist: 'Doctor-Specialist',
        roleDoctor: 'Doctor',
        roleResident: 'Resident',
        roleStudent: 'Student',
        roleNurse: 'Nurse',
        specialty: 'Specialty',
        specialtyGeneralSurgeon: 'General Surgeon',
        specialtyGastroenterologist: 'Gastroenterologist',
        specialtyRadiologist: 'Radiologist',
        specialtyOncologist: 'Oncologist',
        specialtyPathologist: 'Pathologist',
        specialtyOther: 'Other specialty',
        specialtyOtherText: 'Other',
        sector: 'Work Sector',
        sectorState: 'State',
        sectorPrivate: 'Private',
        institution: 'Institution',
        email: 'Email',
        phone: 'Phone',
        enterFirstName: 'Enter your first name',
        enterLastName: 'Enter your last name',
        enterInstitution: 'Enter your institution',
        enterPhone: 'Enter your phone number',
        enterSpecialtyOther: 'Enter your other specialty',
        selectRole: 'Select role',
        selectSpecialty: 'Select specialty',
        selectSector: 'Select sector',
        firstNameRequired: 'First name is required',
        lastNameRequired: 'Last name is required',
        genderRequired: 'Gender is required',
        roleRequired: 'Role is required',
        specialtyRequired: 'Specialty is required',
        specialtyOtherRequired: 'Other specialty is required',
        sectorRequired: 'Sector is required',
        institutionRequired: 'Institution is required',
        emailRequired: 'Email is required',
        phoneRequired: 'Phone number is required',
        invalidEmail: 'Please enter a valid email address',
        cancel: 'Cancel',
        sending: 'Sending...',
        requestSentSuccess: 'Request sent successfully!',
        requestRecorded: 'Your request has been recorded. We will contact you soon.',
        ok: 'OK',
        requestError: 'Error!',
        requestNotSent: 'Request was not sent. Please try again.',
        // Contact Page
        contactTitle: 'If you have any questions,',
        contactSubtitle: 'we are here!',
        loadingContactInfo: 'Loading contact information...',
        errorLoadingContactInfo: 'Error loading contact information',
        checkApiServer: 'Please check if the API server is running',
        noContactInfo: 'No contact information available',
        noSocialMedia: 'No social media links available',
        loadingScene: 'Loading 3D Scene...',
        failedToLoadScene: 'Failed to load 3D scene',
        retry: 'Retry',
        firstName: 'First Name',
        lastName: 'Last Name',
        email: 'Email',
        phone: 'Phone',
        message: 'Your Message',
        enterFirstName: 'Enter your first name',
        enterLastName: 'Enter your last name',
        enterEmail: 'Enter your email address',
        enterPhone: 'Enter your phone number',
        enterMessage: 'Enter your message',
        firstNameRequired: 'First name is required',
        lastNameRequired: 'Last name is required',
        emailRequired: 'Email is required',
        phoneRequired: 'Phone number is required',
        messageRequired: 'Message is required',
        invalidEmail: 'Please enter a valid email address',
        sending: 'Sending...',
        sendMessage: 'Send Message',
        messageSentSuccess: 'Your message was sent successfully!',
        messageRecorded: 'Your message has been recorded. We will contact you soon.',
        ok: 'OK',
        messageError: 'Error!',
        messageNotSent: 'Message was not sent. Please try again.',
        findUsOnMap: 'Find Us on Map',
        languages: 'Languages',
        // Account Benefits
        membershipRequiredMessage: 'You need to be a member to access scientific materials.',
        noScientificMaterials: 'No scientific materials available.',
        loadingMaterials: 'Loading...',
        // Auth Pages
        loginTitle: 'Login',
        loginFailed: 'Login failed',
        emailLabel: 'Email',
        emailPlaceholder: 'Enter your email',
        passwordLabel: 'Password',
        passwordPlaceholder: 'Enter your password',
        forgotPasswordLink: 'Forgot your password?',
        loggingIn: 'Logging in...',
        loginButton: 'Login',
        noAccount: "Don't have an account?",
        registerLink: 'Register',
        forgotPasswordTitle: 'Forgot Your Password?',
        resetLinkSent: 'Password reset link has been sent to your email. Please check your inbox.',
        failedToSendLink: 'Failed to send reset link',
        sending: 'Sending...',
        sendResetLink: 'Send Reset Link',
        backToLogin: 'Back to',
        registerTitle: 'Register',
        passwordsDoNotMatch: 'Passwords do not match',
        invalidEmailFormat: 'Invalid email format',
        invalidPhoneFormat: 'Invalid phone format. Example: +994501234567',
        finMustBe7Chars: 'FIN must be 7 characters',
        locationRequired: 'Location is required',
        accountCreated: 'Account created! Redirecting to login...',
        registrationFailed: 'Registration failed',
        registering: 'Registering...',
        haveAccount: 'Already have an account?',
        loginLink: 'Login',
        firstNameLabel: 'First Name',
        firstNamePlaceholder: 'Enter your first name',
        lastNameLabel: 'Last Name',
        lastNamePlaceholder: 'Enter your last name',
        phoneLabel: 'Phone',
        phonePlaceholder: 'Enter your phone number',
        positionLabel: 'Position',
        positionPlaceholder: 'Enter your position',
        finCodeLabel: 'FIN Code',
        finCodePlaceholder: 'Enter your ID card FIN code',
        locationLabel: 'Location',
        locationPlaceholder: 'Enter your location',
        clinicLabel: 'Clinic',
        clinicPlaceholder: 'Enter your clinic (optional)',
        passwordConfirmLabel: 'Confirm Password',
        passwordConfirmPlaceholder: 'Enter your password',
        resetPasswordTitle: 'Set New Password',
        invalidResetToken: 'Invalid or missing reset token',
        passwordResetSuccess: 'Password reset successfully! Redirecting to login...',
        resetPasswordFailed: 'Failed to reset password',
        resetting: 'Resetting...',
        resetPasswordButton: 'Reset Password',
        newPasswordLabel: 'New Password',
        newPasswordPlaceholder: 'Enter your new password',
        confirmPasswordLabel: 'Confirm Password',
        confirmPasswordPlaceholder: 'Re-enter your password',
        passwordMinLength: 'Password must be at least 6 characters'
    },
    ru: {
        // About page
        doctors: 'Врачи',
        events: 'Мероприятия',
        aboutTitle: 'Общественная Ассоциация Гепато-Панкреато-Билиарных Хирургов Азербайджана',
        aboutDescription: 'Общественная Ассоциация Гепато-Панкреато-Билиарных Хирургов Азербайджана - это научно-общественная организация, объединяющая специалистов, работающих в области диагностики и хирургии заболеваний печени, желчевыводящих путей и поджелудочной железы.\n\nОсновная цель Ассоциации - способствовать обмену знаниями и опытом в области ГПБ, поддерживать профессиональное развитие и вносить вклад в развитие этой области в стране. Ассоциация организует семинары, научные конференции и обучение, устанавливая как местное, так и международное сотрудничество. Поддержка молодых хирургов и специалистов, внедрение современных хирургических методов и продвижение научных исследований также являются одними из основных направлений нашей деятельности.\n\nКак Общественная Ассоциация ГПБ Хирургов Азербайджана, мы работаем как профессиональное общество, чтобы добавить ценности системе здравоохранения и улучшить качество жизни пациентов.',
        // Header navigation
        home: 'Главная',
        about: 'О нас',
        events: 'Мероприятия',
        members: 'Руководство',
        membersNav: 'Участники',
        membersPageTitle: 'Участники',
        membersListTitle: 'Наши участники',
        membersPosition: 'Должность',
        membersClinic: 'Клиника',
        membersDefaultClinic: 'Баку, Азербайджан',
        membersEmpty: 'Участники не найдены',
        membersLoginRequired: 'Войдите, чтобы увидеть участников.',
        login: 'Войти',
        gallery: 'Галерея',
        blog: 'Блог',
        contact: 'Контакты',
        joinMember: 'Стать участником',
        leaveMember: 'Покинуть участника',
        // Employee detail page
        whoAmI: 'Кто Я?',
        pathToMedicine: 'Путь к Медицине',
        now: 'сейчас',
        more: 'Больше',
        less: 'Меньше',
        loadingEmployeeDetails: 'Загрузка данных сотрудника...',
        errorLoadingEmployee: 'Ошибка загрузки сотрудника',
        employeeNotFound: 'Сотрудник не найден',
        backToEmployees: 'Назад к Сотрудникам',
        // Employee page
        loadingEmployees: 'Загрузка сотрудников...',
        errorLoadingEmployees: 'Ошибка загрузки сотрудников',
        retry: 'Повторить',
        noEmployeesFound: 'Сотрудники не найдены',
        // Event detail page
        conferenceSpeakers: 'Спикеры конференции',
        conferencePlanning: 'Планирование конференции',
        gallery: 'Галерея',
        loadingData: 'Загрузка данных...',
        noInformationAvailable: 'Информация недоступна',
        noTimelineData: 'Данные временной шкалы недоступны',
        // Events page
        register: 'Зарегистрироваться',
        viewDetails: 'Подробнее',
        allEvents: 'Все',
        events: 'Мероприятия',
        all: 'Все',
        searchEvents: 'Поиск мероприятий...',
        refreshCalendar: 'Обновить календарь',
        loading: 'Загрузка...',
        error: 'Ошибка',
        // Home page
        buyTicket: 'Купить билет',
        viewDetailsHome: 'Подробнее',
        upcomingEvents: 'Предстоящие',
        allEventsHome: 'Мероприятия',
        viewAll: 'Посмотреть все',
        recentEvents: 'Последние',
        pastEvents: 'Прошедшие мероприятия',
        ourMembers: 'Наши',
        members: 'Руководство',
        price: 'Цена',
        noEventsFound: 'Мероприятия не найдены',
        noPastEventsFound: 'Прошедшие мероприятия не найдены',
        basketTitle: 'Корзина',
        basketEmpty: 'Корзина пуста',
        basketEvent: 'Событие',
        basketDate: 'Дата',
        basketPrice: 'Цена',
        basketStatus: 'Статус',
        basketBought: 'Куплено',
        viewMore: 'Подробнее',
        // Footer
        home: 'Главная',
        about: 'О нас',
        events: 'Мероприятия',
        employees: 'Руководство',
        gallery: 'Галерея',
        blog: 'Блог',
        contact: 'Контакты',
        websiteByPrefix: 'Веб-сайт разработан компанией',
        websiteBySuffix: '.',
        copyright: 'Авторские права @2025. Все права защищены. AHBPCA',
        // Months
        january: 'Январь',
        february: 'Февраль',
        march: 'Март',
        april: 'Апрель',
        may: 'Май',
        june: 'Июнь',
        july: 'Июль',
        august: 'Август',
        september: 'Сентябрь',
        october: 'Октябрь',
        november: 'Ноябрь',
        december: 'Декабрь',
        // Price
        free: 'Бесплатно',
        // Request Modal
        sendRequest: 'Отправить заявку',
        firstName: 'Имя',
        lastName: 'Фамилия',
        gender: 'Пол',
        male: 'Мужской',
        female: 'Женский',
        role: 'Статус',
        roleDoctorSpecialist: 'Врач-специалист',
        roleDoctor: 'Врач',
        roleResident: 'Ординатор',
        roleStudent: 'Студент',
        roleNurse: 'Медсестра',
        specialty: 'Специальность',
        specialtyGeneralSurgeon: 'Общий хирург',
        specialtyGastroenterologist: 'Гастроэнтеролог',
        specialtyRadiologist: 'Радиолог',
        specialtyOncologist: 'Онколог',
        specialtyPathologist: 'Патолог',
        specialtyOther: 'Другая специальность',
        specialtyOtherText: 'Другое',
        sector: 'Сектор работы',
        sectorState: 'Государственный',
        sectorPrivate: 'Частный',
        institution: 'Учреждение',
        email: 'Email',
        phone: 'Телефон',
        enterFirstName: 'Введите ваше имя',
        enterLastName: 'Введите вашу фамилию',
        enterInstitution: 'Введите ваше учреждение',
        enterPhone: 'Введите ваш номер телефона',
        enterSpecialtyOther: 'Введите вашу другую специальность',
        selectRole: 'Выберите статус',
        selectSpecialty: 'Выберите специальность',
        selectSector: 'Выберите сектор',
        firstNameRequired: 'Имя обязательно',
        lastNameRequired: 'Фамилия обязательна',
        genderRequired: 'Пол обязателен',
        roleRequired: 'Статус обязателен',
        specialtyRequired: 'Специальность обязательна',
        specialtyOtherRequired: 'Другая специальность обязательна',
        sectorRequired: 'Сектор обязателен',
        institutionRequired: 'Учреждение обязательно',
        emailRequired: 'Email обязателен',
        phoneRequired: 'Номер телефона обязателен',
        invalidEmail: 'Пожалуйста, введите корректный email адрес',
        cancel: 'Отмена',
        sending: 'Отправляется...',
        requestSentSuccess: 'Заявка успешно отправлена!',
        requestRecorded: 'Ваша заявка записана. Мы свяжемся с вами в ближайшее время.',
        ok: 'ОК',
        requestError: 'Ошибка!',
        requestNotSent: 'Заявка не была отправлена. Пожалуйста, попробуйте снова.',
        // Contact Page
        contactTitle: 'Если у вас есть вопросы,',
        contactSubtitle: 'мы здесь!',
        loadingContactInfo: 'Загрузка контактной информации...',
        errorLoadingContactInfo: 'Ошибка загрузки контактной информации',
        checkApiServer: 'Пожалуйста, проверьте, запущен ли API сервер',
        noContactInfo: 'Контактная информация недоступна',
        noSocialMedia: 'Ссылки на социальные сети недоступны',
        loadingScene: 'Загрузка 3D сцены...',
        failedToLoadScene: 'Не удалось загрузить 3D сцену',
        retry: 'Повторить',
        firstName: 'Имя',
        lastName: 'Фамилия',
        email: 'Email',
        phone: 'Телефон',
        message: 'Ваше сообщение',
        enterFirstName: 'Введите ваше имя',
        enterLastName: 'Введите вашу фамилию',
        enterEmail: 'Введите ваш email адрес',
        enterPhone: 'Введите ваш номер телефона',
        enterMessage: 'Введите ваше сообщение',
        firstNameRequired: 'Имя обязательно',
        lastNameRequired: 'Фамилия обязательна',
        emailRequired: 'Email обязателен',
        phoneRequired: 'Номер телефона обязателен',
        messageRequired: 'Сообщение обязательно',
        invalidEmail: 'Пожалуйста, введите корректный email адрес',
        sending: 'Отправляется...',
        sendMessage: 'Отправить сообщение',
        messageSentSuccess: 'Ваше сообщение успешно отправлено!',
        messageRecorded: 'Ваше сообщение записано. Мы свяжемся с вами в ближайшее время.',
        ok: 'ОК',
        messageError: 'Ошибка!',
        messageNotSent: 'Сообщение не было отправлено. Пожалуйста, попробуйте снова.',
        findUsOnMap: 'Найдите нас на карте',
        languages: 'Языки',
        // Account Benefits
        membershipRequiredMessage: 'Вам необходимо быть участником, чтобы получить доступ к научным материалам.',
        noScientificMaterials: 'Научные материалы недоступны.',
        loadingMaterials: 'Загрузка...',
        // Auth Pages
        loginTitle: 'Войти',
        loginFailed: 'Не удалось войти',
        emailLabel: 'Email',
        emailPlaceholder: 'Введите ваш email',
        passwordLabel: 'Пароль',
        passwordPlaceholder: 'Введите ваш пароль',
        forgotPasswordLink: 'Забыли пароль?',
        loggingIn: 'Вход...',
        loginButton: 'Войти',
        noAccount: 'Нет аккаунта?',
        registerLink: 'Зарегистрироваться',
        forgotPasswordTitle: 'Забыли Пароль?',
        resetLinkSent: 'Ссылка для сброса пароля отправлена на ваш email. Пожалуйста, проверьте почту.',
        failedToSendLink: 'Не удалось отправить ссылку для сброса',
        sending: 'Отправляется...',
        sendResetLink: 'Отправить Ссылку',
        backToLogin: 'Вернуться к',
        registerTitle: 'Регистрация',
        passwordsDoNotMatch: 'Пароли не совпадают',
        invalidEmailFormat: 'Неверный формат email',
        invalidPhoneFormat: 'Неверный формат телефона. Пример: +994501234567',
        finMustBe7Chars: 'FIN должен состоять из 7 символов',
        locationRequired: 'Адрес обязателен',
        accountCreated: 'Аккаунт создан! Перенаправление на страницу входа...',
        registrationFailed: 'Регистрация не удалась',
        registering: 'Регистрация...',
        haveAccount: 'Уже есть аккаунт?',
        loginLink: 'Войти',
        firstNameLabel: 'Имя',
        firstNamePlaceholder: 'Введите ваше имя',
        lastNameLabel: 'Фамилия',
        lastNamePlaceholder: 'Введите вашу фамилию',
        phoneLabel: 'Телефон',
        phonePlaceholder: 'Введите ваш номер телефона',
        positionLabel: 'Должность',
        positionPlaceholder: 'Введите вашу должность',
        finCodeLabel: 'FIN Код',
        finCodePlaceholder: 'Введите FIN код вашего удостоверения личности',
        locationLabel: 'Адрес',
        locationPlaceholder: 'Введите ваш адрес',
        clinicLabel: 'Клиника',
        clinicPlaceholder: 'Введите вашу клинику (необязательно)',
        passwordConfirmLabel: 'Подтвердите Пароль',
        passwordConfirmPlaceholder: 'Введите ваш пароль',
        resetPasswordTitle: 'Установить Новый Пароль',
        invalidResetToken: 'Неверный или отсутствующий токен сброса',
        passwordResetSuccess: 'Пароль успешно сброшен! Перенаправление на страницу входа...',
        resetPasswordFailed: 'Не удалось сбросить пароль',
        resetting: 'Сброс...',
        resetPasswordButton: 'Сбросить Пароль',
        newPasswordLabel: 'Новый Пароль',
        newPasswordPlaceholder: 'Введите ваш новый пароль',
        confirmPasswordLabel: 'Подтвердите Пароль',
        confirmPasswordPlaceholder: 'Повторно введите пароль',
        passwordMinLength: 'Пароль должен содержать не менее 6 символов'
    }
}

export const useTranslation = () => {
    const { selectedLanguage } = useLanguage()

    const t = (key) => {
        const languageTranslations = translations[selectedLanguage] || translations.aze
        return languageTranslations[key] || key
    }

    return { t }
}
